...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  #right { width:330px; display:flex; flex-direction:column; min-height:0; }
  #tabs { display:flex; gap:4px; padding:10px 12px 0; }
  .tab { flex:1; padding:6px 8px; border-radius:9px; font-size:12px; font-weight:600;
         text-align:center; cursor:pointer; color:var(--ink2); }
  .tab.on { background:rgba(42,120,214,.10); color:var(--accent); }
  #pane-node,#pane-chat { flex:1; padding:12px 14px 14px; overflow:auto; min-height:0; }
  #pane-chat { display:none; flex-direction:column; }
  #chatlog { flex:1; overflow:auto; display:flex; flex-direction:column; gap:8px; }
  .msg { max-width:92%; padding:8px 11px; border-radius:12px; font-size:12.5px; line-height:1.5; }
  .msg.me { align-self:flex-end; background:var(--accent); color:#fff; }
  .msg.ai { align-self:flex-start; background:var(--plane); border:1px solid var(--hair); }
  .msg .gid { color:var(--accent); cursor:pointer; text-decoration:underline; }
  .msg.ai .note { display:block; margin-top:5px; color:var(--muted); font-size:11px; }
  #chatform { display:flex; gap:6px; margin-top:10px; }
  #chatform input { flex:1; }
  #chatform button { padding:8px 12px; border:0; border-radius:9px; background:var(--accent);
  ...
  .suggest button { background:rgba(11,11,11,.05); color:var(--ink2); border:0; border-radius:11px;
...
<main>
  ...
  <div class="panel" id="right">
    <div id="tabs">
      <div class="tab on" data-pane="node">Узел</div>
      <div class="tab" data-pane="chat">Ассистент</div>
    </div>
    <div id="pane-node"><p class="hint">Выберите узел на схеме или в списке слева.</p></div>
    <div id="pane-chat">
      <div id="chatlog"></div>
      <div class="suggest" id="suggest"></div>
      <form id="chatform"><input id="chatq" placeholder="Вопрос по сети" autocomplete="off">
...
function card(n) {
  ...
  document.getElementById('pane-node').innerHTML = `
    <h2>${n.gid}</h2>
  ...
  document.querySelectorAll('#pane-node .row').forEach(el =>
    el.onclick = () => select(el.dataset.gid, true));
}
...
// --- ассистент ---
// Страница остаётся самодостаточной: чат включается, только если рядом
// работает локальный эндпоинт. Без него схема, роли и поиск работают как обычно.
const ENDPOINTS = [location.origin.startsWith('http') ? location.origin : null,
                   'http://127.0.0.1:8765'].filter(Boolean);
let api = null;

const SUGGESTIONS = [
  'кого смотреть первым и почему?',
  'каких данных не хватает по этому узлу?',
  'что будет, если убрать этот узел?',
  'возвращаются ли деньги к отправителю?',
];

function setPane(name) {
  document.querySelectorAll('.tab').forEach(t => t.classList.toggle('on', t.dataset.pane === name));
  document.getElementById('pane-node').style.display = name === 'node' ? 'block' : 'none';
  document.getElementById('pane-chat').style.display = name === 'chat' ? 'flex' : 'none';
}
...
function chatMsg(cls, html) {
  ...
  el.innerHTML = html;
  el.querySelectorAll('.gid').forEach(g => g.onclick = () => {
    document.getElementById('scope').value = 'hops2';
    select(g.textContent, true);
    setPane('node');
  });
...
// Номера счетов в ответе кликабельны: из ответа сразу на схему.
function linkGids(text) {
  const esc = text.replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
  return esc.replace(/\b(\d{15,20})\b/g, (m) => byId.has(m) ? `<span class="gid">${m}</span>` : m);
}
...
async function detectApi() {
  for (const base of ENDPOINTS) {
    try {
      ...
      if (r.ok) { api = base; return await r.json(); }
    } catch (e) { /* эндпоинта нет — это нормальный режим */ }
  }
  return null;
}

async function askAssistant(q) {
  if (!q.trim() || !api) return;
  chatMsg('me', linkGids(q));
  ...
  try {
    ...
    if (j.error) { chatMsg('ai', 'Ошибка: ' + j.error); return; }
    const notes = (j.warnings || []).map(w => `<span class="note">! ${w}</span>`).join('');
    const tools = j.tools_used?.length ? `<span class="note">инструменты: ${j.tools_used.join(', ')}</span>` : '';
    chatMsg('ai', linkGids(j.answer) + tools + notes);
  } catch (e) {
    chatMsg('ai', 'Не удалось получить ответ: ' + e);
  } finally {
    btn.disabled = false;
  }
}
...
document.getElementById('chatform').onsubmit = e => {
  e.preventDefault();
  askAssistant(document.getElementById('chatq').value);
};

// Deep link на вкладку и на готовый вопрос: удобно репетировать демо.
function chatFromHash() {
  const m = location.hash.match(/[#&]ask=([^&]+)/);
  return m ? decodeURIComponent(m[1]) : null;
}
if (/[#&]tab=chat/.test(location.hash) || chatFromHash()) setPane('chat');

detectApi().then(info => {
  const pane = document.getElementById('chatlog');
  const sug = document.getElementById('suggest');
  if (!info) {
    document.getElementById('chatform').style.display = 'none';
    pane.innerHTML = `<p class="hint">Ассистент недоступен: схема и поиск работают без него.<br><br>
      Чтобы задавать вопросы по сети на естественном языке, запустите
      <code>moneygraph-serve</code> и обновите страницу.</p>`;
    return;
  }
  const mode = info.llm ? `LLM (${info.model})` : 'детерминированный режим';
  pane.innerHTML = `<p class="hint">Ассистент подключён, ${mode}.
    Номера счетов в ответах кликабельны — открывают узел на схеме.</p>`;
  SUGGESTIONS.forEach(q => {
    const b = document.createElement('button');
    b.textContent = q;
    b.onclick = () => askAssistant(selected ? q.replace('этому узлу', selected)
      .replace('этот узел', selected) : q);
    sug.appendChild(b);
  });
  const preset = chatFromHash();
  if (preset) { setPane('chat'); askAssistant(preset); }
});
